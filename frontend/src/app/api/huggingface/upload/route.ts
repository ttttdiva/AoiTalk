...
export async function POST(request: NextRequest) {
  ...
  const parsed = typeof virtualPath === "string" ? parseHfPath(virtualPath) : null;
  if (!parsed || parsed.kind !== "repo" || !parsed.repoId || !parsed.repoType) {
    return NextResponse.json({ detail: "HFリポジトリ内で実行してください" }, { status: 400 });
  }
  if (!parsed.accountId) {
    return NextResponse.json({ detail: "書き込み用HFアカウントが関連付けられていません" }, { status: 403 });
  }
  const resolved = resolveToken(parsed.accountId);
  if (!resolved) {
    return NextResponse.json({ detail: "HFアカウントが見つかりません" }, { status: 403 });
  }

  const files = form.getAll("files").filter((item): item is File => item instanceof File);
  const filePaths = form.getAll("filePaths");
  if (files.length === 0) {
    return NextResponse.json({ detail: "アップロードするファイルがありません" }, { status: 400 });
  }

  const basePath = parsed.subPath?.replace(/^\/+|\/+$/g, "") || "";
  if (basePath && !normalizeRelativePath(basePath)) {
    return NextResponse.json({ detail: "アップロード先のパスが不正です" }, { status: 400 });
  }
  const destinations = files.map((file, index) => {
    const supplied = filePaths[index];
    const relative = normalizeRelativePath(
      typeof supplied === "string" ? supplied : path.posix.basename(file.name.replace(/\\/g, "/")),
    );
    return relative ? [basePath, relative].filter(Boolean).join("/") : null;
  });
  ...
  const results: PromiseSettledResult<void>[] = destinations.map((destination) =>
    !destination || duplicatePaths.has(destination)
      ? { status: "rejected", reason: new Error("invalid or duplicate destination") }
  ...
  const failures = results
    .map((result, index) => ({ result, name: files[index].name }))
    .filter((item) => item.result.status === "rejected")
    .map((item) => ({ name: item.name, error: "HFへのアップロードに失敗しました" }));
  ...
  return NextResponse.json(
    {
      ...
      failures,
      ...(successCount === 0 ? { detail: "HFへのアップロードに失敗しました" } : {}),
    },
