...
class CharacterSwitchDetector(KeywordDetectorBase):
    ...
    def __init__(self, enabled: bool = True, config: Optional[Any] = None):
        ...
        logger.info(
            f"[CharacterSwitchDetector] 新しいインスタンスを作成しました (ID: {id(self)})"
        )
        logger.info(
            f"[CharacterSwitchDetector] SelectionModeState インスタンスID: {id(self.selection_mode_state)}"
        )
        ...
        self.character_map: Dict[str, Tuple[str, str]] = (
            {}
        )  # {alias: (character_name, yaml_filename)}
        ...
        self.end_pattern = re.compile(
            "|".join(f"({p})" for p in self.end_conversation_patterns), re.IGNORECASE
        )
    ...
    def _load_character_aliases(self):
        ...
        try:
            ...
            logger.error(f"DBからキャラクター一覧の取得に失敗: {e}")
            return

        for char_data in characters:
            char_name = char_data.get("name")
            slug = char_data.get("slug", "")
            aliases = char_data.get("recognition_aliases", [])
            ...
            if char_name and aliases:
                ...
                for alias in aliases:
                    ...
                    self.character_map[normalized_alias] = (char_name, slug)
    ...
    def detect(self, text: str) -> KeywordDetectionResult:
        ...
        if self.selection_mode_state.active:
            logger.info(
                f"[CharacterSwitchDetector] 選択モード中のテキスト検出: '{text}' (選択モード: {self.selection_mode_state.active}, インスタンスID: {id(self)})"
            )
        ...
        if match:
            logger.info(
                f"[CharacterSwitchDetector] 会話終了キーワード検出: '{match.group()}' (選択モード: {self.selection_mode_state.active}, インスタンスID: {id(self)})"
            )
    ...
    def _detect_character_name(self, text: str) -> KeywordDetectionResult:
        ...
        return KeywordDetectionResult(
            detected=True,
            action=KeywordAction.PROCESS,
            parameters={"action": "selection_mode_no_match"},
            bypass_llm=True,
        )
    ...
    def process(self, result: KeywordDetectionResult) -> Optional[str]:
        ...
        if action == "enter_selection_mode":
            # キャラクター選択モードに移行
            logger.info(
                f"[CharacterSwitchDetector] 選択モード移行前の状態: {self.selection_mode_state.active} (インスタンスID: {id(self)})"
            )
            self.selection_mode_state.activate()
            logger.info(
                f"[CharacterSwitchDetector] 選択モードをアクティブにしました: {self.selection_mode_state.active} (インスタンスID: {id(self)})"
            )
            ...
            # 利用可能なキャラクターのリストを作成
            character_list = "\n".join(
                [f"• {char}" for char in self.available_characters]
            )
            ...
            # 実際の切り替え処理を実行
            logger.info(
                f"[CharacterSwitchDetector] キャラクター切り替えを実行: {char_name} ({yaml_filename})"
            )
            success = self._apply_character_switch(char_name, yaml_filename)
            logger.info(
                f"[CharacterSwitchDetector] キャラクター切り替え結果: {success}"
            )

            if success:
                # 選択モードを終了
                logger.info(
                    f"[CharacterSwitchDetector] 選択モード終了前の状態: {self.selection_mode_state.active} (インスタンスID: {id(self)})"
                )
                self.selection_mode_state.deactivate()
                logger.info(
                    f"[CharacterSwitchDetector] 選択モードを終了しました: {self.selection_mode_state.active} (インスタンスID: {id(self)})"
                )
            ...
            # 選択モード中でマッチしなかった場合
            logger.info(
                f"[CharacterSwitchDetector] 選択モード中でキャラクターが見つかりませんでした。選択モード継続: {self.selection_mode_state.active}"
            )
            return {
    ...
    def _apply_character_switch(self, character_name: str, yaml_filename: str) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"キャラクター切り替え成功: {self.current_character} -> {character_name} ({yaml_filename})"
                )
            else:
    ...
    def _get_character_config(self, character_name: str) -> Optional[Dict[str, Any]]:
        ...
        # キャラクター名からslugを特定
        slug = None
        for alias, (char_name, char_slug) in self.character_map.items():
        ...
        # DBからキャラクター設定を取得（YAML互換形式で返す）
        try:
            from ....config import Config

            config = Config()
            return config.get_character_config(slug)
        except Exception as e:
