#!/usr/bin/env python3
"""
Absolute filer path support for the unified file explorer.

Provides directory scanning and file serving for local images, videos, and audio
when the file explorer navigates to an absolute filesystem path.
...
def _get_config() -> Dict[str, Any]:
    """Get absolute filer path configuration from environment."""
    global _config_cache
    
    if _config_cache:
        return _config_cache
    
    root_path = os.environ.get('FILER_ROOT_PATH', '')
    thumbnail_cache = os.environ.get('FILER_VIDEO_THUMBNAIL_CACHE', 'cache/video_thumbnails')
    ...
    if _config_cache['root_path'] and _config_cache['root_path'].exists():
        logger.info(f"Filer root path configured: {root_path}")
    else:
        logger.warning(f"Filer root path not configured or does not exist: {root_path}")
...
def get_filer_config() -> Dict[str, Any]:
    """
    Get absolute filer root configuration.
...
def list_folder_contents(path: str = "") -> Dict[str, Any]:
    ...
    Returns:
        {
            "success": True,
            "current_path": "D:\\f\\a\\subfolder",
            "parent_path": "D:\\f\\a" or null,
            "can_go_up": True,
            "folders": [...],
            "files": [...]
        }
    ...
    try:
        for item in sorted(target_path.iterdir(), key=_natural_sort_key):
            ...
            # Resolve actual path (handle .lnk shortcuts pointing to folders)
            actual_item = item
            is_shortcut = False
            ...
            if actual_item.is_dir():
                # Count items in subfolder
                try:
                    item_count = len(list(actual_item.iterdir()))
                except:
                    item_count = 0

                # Get folder thumbnail
                thumbnail = _get_folder_thumbnail(actual_item)

                folder_data = {
                    "name": item.name if is_shortcut else actual_item.name,
                    "path": str(actual_item),
                    "item_count": item_count
                }

                if is_shortcut:
                    folder_data["is_shortcut"] = True
                    folder_data["shortcut_path"] = str(item)
                ...
                # Include images, videos, audio files, and shortcuts
...
def get_filer_mime_type(file_path: Path) -> str:
