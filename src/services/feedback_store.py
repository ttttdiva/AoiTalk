"""
Feedback storage module for AoiTalk (services layer).

Handles saving and loading user feedback on agent responses.
Uses PostgreSQL database for storage (migrated from JSONL).

この保存ロジックはもともと ``src/api/feedback.py`` に存在したが、services 層から
api 層を import する層逆流（services→api）を解消するため services 層へ移設した。
``src/api/feedback.py`` は本モジュールを re-export して従来の公開 API を維持する。
"""
