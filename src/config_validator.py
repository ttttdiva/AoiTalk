...
class Config(BaseModel):
    ...
    def validate_llm_model(cls, v, info):
        ...
        if provider == "openai" and not v.startswith(("gpt-", "o1-")):
            raise ValueError(
                f"OpenAI provider requires model starting with 'gpt-' or 'o1-', got: {v}"
            )
        if provider == "gemini" and not v.startswith("gemini-"):
            raise ValueError(
                f"Gemini provider requires model starting with 'gemini-', got: {v}"
            )
        # OpenRouter and local providers accept provider-qualified names, tags, HF paths, etc.
        return v
...
class ConfigValidator:
    ...
    def load_yaml(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            if not file_path.exists():
                ...
                return load_app_config_sync(file_path)
    ...
    def validate(self, environment: Optional[str] = None) -> bool:
        ...
        # 必須フィールドの存在チェック
        required_fields = ["default_character", "llm_model", "llm_provider"]
        ...
        if default_char:
            if not self._character_config_exists(default_char):
